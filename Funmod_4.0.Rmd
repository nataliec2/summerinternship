...
z <- 0.1 #cost of being a mutualist per unit of uptake rate* 
...
l_x <- 0.2 # loss back to R pool per nutrient pool unit, let resident and mutant to have the same loss
l_y <- 0.2
...
if(is.na(numberofreps)){print("#ofreps is not a number")} 
 ...
 par(mar = c(4,4,4,4))
  plot(Mutant_percentage4, border= NA, fmt.cell='%.3f', fmt.key ="%.2f", breaks = c(-0.1,0.011,.99,1),xlab= "Mutant Uptake Rate, u_n", ylab = "Resident Uptake Rate, u_m", col = c('white','grey','black') ) 
```

# Next lets see how the host is doing?
