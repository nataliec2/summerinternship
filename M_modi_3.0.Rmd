...
r0 <- function(u_m, R, M, H){((u_m)*R*M)/(H*(k + (u_m*R*M/H)))}
r <- function(u_m, u_n, R, N, M){((u_m*M*R) + (u_n*R*N))/((H)*(k + ((u_m*R*M)/H) + ((u_n*R*N)/H)))}
```
...
# Code to visualize changes in host abundance, do not run until you rerun previous and it starts making sense.
```{r}
...
#H0 ; R0 ; M_i ; N_i
 ...
 plot(Host_ab_3.0,border= NA, fmt.key ="%.3f", fmt.cell = "%.3f", xlab= "Mutant Uptake Rate, u_n", ylab = "Resident Uptake Rate, u_m") #col = c('mediumpurple4','mediumorchid4','mediumpurple3','mediumpurple','mediumpurple2','mediumpurple1','mediumorchid3','mediumorchid1')) ,breaks = c(1.729, 1.74,1.75,1.76,1.77,1.78,1.79,1.8,1.82)
...
for(k in 2:length(tset)){
    ...
    dR <- (D*(R_0 - R) - ((u_m)*R*M)/(H*(k + (u_m*R*M/H))) + (s*H)/c)*dt
    dH <- (c*(((u_m)*R*M)/(H*(k + (u_m*R*M/H))))*H  - (s*H) - ((fun(u_m)*C_a) + C_c)*M*H  )*dt
    dM <- (b*(1-(u_m/6))*v*(C_c + (fun(u_m)*C_a))*H*M - l*M)*dt
