...
# Tryna make Fidelity a saturating function of fungal uptake rate. 
```{r}
...
mutset1.2 <- matrix(NaN,nrow = 20, ncol = 20, dimnames = list(seq(1/20, 1, 1/20), seq(1/20, 1, 1/20))) #name rows to be u_m ; cols to be u_n. made a matrix thingie yay

dt <- 0.01

for(i in 1:nrow(mutset1.2)){ #mutset1.6: records every individual who won. nrow here is analogous to "length" command.
u_m <-  as.numeric(rownames(mutset1.2)[i]) 
...
#H0 ; R0 ; M_i ; N_i
 ...
 for(j in 1: ncol(mutset1.2)){
    u_n <- as.numeric(colnames(mutset1.2)[j]) # do whole while loop, initialize u_n and u_m to actual numbers, store delta in the matrix. 
 ...
 # mutset1.2[[i, j]] <- delta #records delta, where delta is intermediate coexistance occured. 
...
# }
   ...
   if (delta >= 0.995){  mutset1.2[[i,j]] <- 1  }
   if(delta <= 0.005){mutset1.2[[i,j]] <- 0  }
   } 
