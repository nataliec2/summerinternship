...
mutset1.1 <- matrix(NaN,nrow = 41, ncol = 41, dimnames = list(seq(0, 2, 1/20), seq(0, 2, 1/20))) #name rows to be u_m ; cols to be u_n. made a matrix thingie yay
#mutset1.1[1] <- 0.7

dt <- 0.01

for(i in 1:nrow(mutset1.1)){ #mutset1.6: records every individual who won. nrow here is analogous to "length" command.
u_m <-  as.numeric(rownames(mutset1.1)[i]) 
...
#H0 ; R0 ; M_i ; N_i
 ...
 for(j in 1: ncol(mutset1.1)){
    u_n <- as.numeric(colnames(mutset1.1)[j]) # do whole while loop, initialize u_n and u_m to actual numbers, store delta in the matrix. 
 ...
 # mutset1.1[[i, j]] <- delta #records delta, where delta is intermediate coexistance occured. 
...
if(is.na(numberofreps)){print("#ofreps is not a number")} #1st:maybe step size is too large - dt. Check with is.na function. 
 ...
 #print(u_m)
  #print(u_n)
   ...
   if (delta >= 0.995){  mutset1.1[[i,j]] <- 1  }
   if(delta <= 0.005){mutset1.1[[i,j]] <- 0  }
   } 
