...
# Mutation steps, records mutant percentage. 
```{r}
mutset1.5 <- matrix(NaN,nrow = 20, ncol = 20, dimnames = list(seq(1/20, 1, 1/20), seq(1/20, 1, 1/20))) #name rows to be u_m ; cols to be u_n. made a matrix thingie yay

dt <- 0.01

for(i in 1:nrow(mutset1.5)){ #mutset1.5: records every individual who won. nrow here is analogous to "length" command.
u_m <-  as.numeric(rownames(mutset1.5)[i]) 
...
M_i <- M.simu1[length(tset)]
 ...
 for(j in 1: ncol(mutset1.5)){
    u_n <- as.numeric(colnames(mutset1.5)[j]) # do whole while loop, initialize u_n and u_m to actual numbers, store delta in the matrix. 
 ...
 #change according to what the resident equilibrates to.. 
  N <- .01*M_i 
  M <- .99*M_i
  H <- H0
  R <- R0
...
for(i in 1:nrow(mutset1.23)){ #mutset1.6: records every individual who won. nrow here is analogous to "length" command.
...
M_i <- M.simu1[length(tset)]
 ...
 for(j in 1: ncol(mutset1.23)){
    u_n <- as.numeric(colnames(mutset1.23)[j]) # do whole while loop, initialize u_n and u_m to actual numbers, store delta in the matrix. 
...
# Plot Graph Double Wheeee
```{r}
plot(x = mutset1.5, y = mutset1.23, xlab = '(Resident Uptake Rate)*(Resident Percentage)', ylab = 'Host Abundance',lwd=2,las=3)
```
