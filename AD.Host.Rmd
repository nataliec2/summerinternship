...
# This Markdown is ori. 

# Comparing a combination of u values, via a matrix, here fidelity is represented as u.
```{r}
...
#looking at squareroot for u values ranging from 0-2. 
#experiment with functions that will produce host increaments, make it a saturating function, extending u_m and u_n to to be zero(not mutualistic at all). 

```


```{r}
mutset1.12 <- matrix(NaN,nrow = 20, ncol = 20, dimnames = list(seq(1/20, 1, 1/20), seq(1/20, 1, 1/20))) #name rows to be u_m ; cols to be u_n. made a matrix thingie yay
#mutset1.12[1] <- 0.7

dt <- 0.01

for(i in 1:nrow(mutset1.12)){ #mutset1.12: records every individual who won. nrow here is analogous to "length" command.
...
mutset1.12[[i, j]] <- H #records delta, where delta is intermediate coexistance occured. 
 ...
 print(round(mutset1.12, 3))
library(plot.matrix)
 Host_abundance.bi<- mutset1.12 
plot(Host_abundance.bi,border= NA,breaks = c(1.2,1.302,1.396,1.493,1.516,1.7,1.83), fmt.key ="%.5f", fmt.cell = "%.3f", xlab= "u_n", ylab = "u_m", col = c('lightcyan','lightskyblue1','lightskyblue2','skyblue', 'skyblue1','skyblue3') )
```


```{r}
mutset1.13 <- matrix(NaN,nrow = 20, ncol = 20, dimnames = list(seq(1/20, 1, 1/20), seq(1/20, 1, 1/20))) #name rows to be u_m ; cols to be u_n. made a matrix thingie yay
mutset1.13[1] <- 0.7

dt <- 0.01

for(i in 1:nrow(mutset1.13)){ #mutset1.6: records every individual who won. nrow here is analogous to "length" command.
u_m <-  as.numeric(rownames(mutset1.13)[i]) 
...
#H0 ; R0 ; M_i ; N_i
 ...
 dt <- 0.01
      #added equilibrated resident population size. starting from **
 for(j in 1: ncol(mutset1.13)){
    u_n <- as.numeric(colnames(mutset1.13)[j]) # do whole while loop, initialize u_n and u_m to actual numbers, store delta in the matrix. 
 ...
 #simulates dynamics initial.
  ...
  R <- R0
  #print(u_m); print(u_n); print(R); print(H); print(dt)
 # mutset1.13[[i, j]] <- delta #records delta, where delta is intermediate coexistance occured. 
 ...
 #}
   ...
   #if (delta >= 0.995){  mutset1.13[[i,j]] <- 1  }
   #if(delta <= 0.005){mutset1.13[[i,j]] <- 0  }
   } 
...
# print("mutset1.13=")
 # print(mutset1.13)
 print(round(mutset1.13, 3))
 
 library(plot.matrix)
 Host_abundance<- mutset1.13 
 plot(Host_abundance,border= NA,breaks = c(1.2,1.296,1.4,1.5,1.6,1.7,1.81), fmt.key ="%.5f", fmt.cell = "%.3f", xlab= "u_n", ylab = "u_m", col = c('lightcyan','lightskyblue1','lightskyblue2','skyblue', 'skyblue1','skyblue3') )
```